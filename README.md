...
1. **先跑 `/dy_img_test`**（管理员，在出问题的会话里执行）：它会发一张即时生成的极小 PNG（约 136 字节）
   ...
   - 极小图也失败 → 问题在协议端（NapCat/QQ）的图片通道本身，**与图片无关**，
     去查 NapCat 登录态/版本、账号风控；
   - 极小图成功、卡片图失败 → 是卡片图本身的问题，看下面第 3 条的格式判断；
   - 卡片图显示「无本地缓存」→ 本进程还没渲染过卡片，先在出问题的会话执行
     `/dy_test <抖音用户>`（不会保存订阅）强制渲染一次，再重跑 `/dy_img_test`；
2. `rai` 开关、上次卡片图的**格式与体积**都可以在 `/dy_status` 里直接看到；
3. **看日志里那一行 `卡片渲染成功` / `卡片渲染结果...`**：
   - `卡片渲染成功: <路径> (JPEG, 120 KB)` → 图片本身有效，若仍失败就是**体积**问题，
     把 `services/renderer.py` 里 `CARD_RENDER_OPTIONS` 的 `quality` 调低（如 50）或缩小卡片宽度；
   - `卡片渲染结果不是有效图片 (格式=未知, 0 字节)` / `数据不完整(疑似被截断)` →
     **AstrBot 的文转图（t2i）返回了空内容或错误内容**，与协议端和图片体积都无关。
     AstrBot 的 `download_image_by_url` 不校验 HTTP 状态码与内容，会把空响应照样存成
     `data/temp/io_temp_img_*.jpg`（0 字节），插件再把它 base64 发给 QQ，必然失败。
     处理办法：检查/更换 AstrBot 的文转图端点，或[自部署 t2i 服务](https://docs.astrbot.app/others/self-host-t2i.html)；
     在此之前建议先把本插件的 `rai` 关掉，走纯文本推送；
     这类无效图片插件会**主动放弃并降级为纯文本**，不会再发出注定失败的请求；
   - `/dy_status` 的「上次卡片图」一行会直接显示 `❌ 不是有效图片 (格式=未知, 0 字节)`，
     不用翻日志；
