...
### Q13: 日志里偶尔出现 `amagi 请求失败 (code=500): 抖音数据获取失败 ({})`，是我 Cookie 过期了吗？

**基本不是**，这是**上游偶发失败**（amagi → 抖音网页接口抽风），实测特征：

- 发生率约 **1% 的轮询**，且是**散落的**（某天 16 次分布在 11 个不同小时里，不是成片失败）；
- 同一时刻直接打桥接接口往往**秒回正常**（实测连续 6 次全部成功、每次 0.6 秒返回 18 条作品）；
- 与机器负载无关（失败时刻机器完全空闲），Cookie 里的 `sessionid`/`ttwid` 等也都在。

**v1.3.1 起插件会自动兜住它**：

1. **自动重试**：桥接返回 `code>=500` 时自动隔 1 秒、2.5 秒各重试一次（网络超时**不**重试，避免拖长轮询周期）；
   绝大多数抖动因此对用户完全无感，日志里只会多一行 `amagi 上游偶发失败, 1s 后重试`。
2. **日志分级**：单个订阅项**首次**失败只记 `WARNING`（并提示"下一轮会重试"），**连续 ≥3 次**才升级为 `ERROR`，
   恢复后补一行 `已恢复正常`，不再被 1% 的抖动刷屏。
3. **可观测**：`/dy_status` 增加 `上游重试: ↻ 上游偶发失败已重试 N 次 (恢复 M 次)` 与 `连续失败项`。

**什么时候才需要你出手**：如果 `/dy_status` 里 `连续失败项` 一直挂着（比如连续十几分钟都是同一个用户），
或者重试后的报错变成了"Cookie 未配置/登录失效"，那就去更新 `douyin_cookie`，再 `/dy_bridge_restart`。

> 顺带一提：**不会丢更新**。失败的那一轮只是跳过检查，下一轮成功时仍会拿最新作品和基线（`last_video_id` /
> `last_video_time`）比对，该推的照推 —— 只是晚一个轮询周期。
